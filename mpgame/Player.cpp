...
/*Ben Lo's Change Log:  Recently added in a '*KILL_COUNT' to both the forward movespeed and the sidespeed
                        Added gameLocal.mpGame.playerState[this -> entityNumber].fragCount to the speed adjustment
						bl233[1] - increases speed based kill count
						bl233[2] - increases crouch speed based on kill count
						bl233[5] - lowers max health based on kill count
						bl233[4] - lowers max armor based on kill count*/
...
void idInventory::RestoreInventory( idPlayer *owner, const idDict &dict ) {
	...
	// health/armor
	maxHealth		= dict.GetInt( "maxhealth", "100" ) - gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount*1.5;//bl233[5]
	armor			= dict.GetInt( "armor", "50" );
	maxarmor		= dict.GetInt( "maxarmor", "100" ) - gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount*1.5;//bl233[4]
...
float idPlayer::PowerUpModifier( int type ) {
	...
	if ( PowerUpActive( POWERUP_QUADDAMAGE ) ) {
		switch( type ) {
			//bl233[22] - speed increases by a flat amount
				
			case PMOD_MELEE_DAMAGE: {
				 physicsObj.SetMaxJumpHeight( pm_jumpheight.GetFloat() + 100); //bl233[9]
				/*mod *= 3.0f;
				break;*/
			}
	...
	if ( PowerUpActive( POWERUP_HASTE ) ) {
		switch ( type ) {
			//bl233[27] - adds 200 to the health
			case PMOD_SPEED:
				inventory.maxHealth += 200;
				/*mod *= 1.3f;
				break;
...
*/
		...
		//bl233[3] - Added a Debug code to add kills to the frag count; binded to the key 'b'
		case IMPULSE_23: {
			if ( gameLocal.isClient || entityNumber == gameLocal.localClientNum ) {
...
void idPlayer::AdjustSpeed( void ) {
	...
	//bl233[22]
	speed = gameLocal.mpGame.playerState[this -> entityNumber].fragCount * 10 + speed;//bl233[1]
	...
	physicsObj.SetSpeed( speed, pm_crouchspeed.GetFloat() + gameLocal.mpGame.playerState[this -> entityNumber].fragCount*10 );//bl233[2]
}
...
void idPlayer::Move( void ) {
	...
	physicsObj.SetMaxJumpHeight( pm_jumpheight.GetFloat()+48 * gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount ); //bl233[9]
...
void idPlayer::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
	...
	if ( pfl.dead ) {
		pfl.pain = true;
		//bl233[20] - if player dies, reset kills to 0
		gameLocal.mpGame.playerState[this -> entityNumber].fragCount = 0;
		return;
	}
