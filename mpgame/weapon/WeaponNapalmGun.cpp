	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			if ( wsfl.zoom ) {
				...
				//bl233[17] - when alt trigger is true, the weapon's spread will spread
				Attack ( true, 1, spread+gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount, 0, 1.0f );
				PlayAnim ( ANIMCHANNEL_ALL, "idle", parms.blendFrames );
				//fireHeld = true;
			} else {
				nextAttackTime = gameLocal.time + (fireRate * owner->PowerUpModifier ( PMOD_FIRERATE ));
				//bl233[16] - fires multiple napalms
				Attack ( false, 1+gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount, spread, 0, 1.0f );
