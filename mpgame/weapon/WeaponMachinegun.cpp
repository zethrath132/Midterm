	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			if ( wsfl.zoom ) {
				...
				//bl233[19] - alt fire becomes a sniper
				Attack ( true, 1, spreadZoom, 0, 1.0f*gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount);
				fireHeld = true;
			} else {
				//bl233[18] - fire rate increases based on kill
				nextAttackTime = gameLocal.time + (fireRate * owner->PowerUpModifier ( PMOD_FIRERATE )-gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount);
				if(fireRate <= 0)
				{
					fireRate = fireRate;
				}
