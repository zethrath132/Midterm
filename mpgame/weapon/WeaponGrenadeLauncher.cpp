...
stateResult_t rvWeaponGrenadeLauncher::State_Fire ( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			...
			//bl233[11] - as kills go up, so do the amount of grenades spawn and spread
			Attack ( false, 1+gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount, spread+(gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount/2), 0, 1.0f );
