...
stateResult_t rvWeaponNailgun::State_Fire( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		...
		case STAGE_FIRE:
			...
			if ( wsfl.zoom ) {
				//bl233[14] - spread for alt fire decreases based on kills but will not fall lower than 0.  Damage is also increased
				Attack ( true, 1, spread - gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount, 0.0f, 1.0f+(gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount/2) );
				if ( spread <= 0)
				{
					spread = 1;
				}
				...
				//bl233[15] - spread increases based on kills
				Attack ( false, 1, spread+gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount, 0.0f, 1.0f );
