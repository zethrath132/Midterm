...
stateResult_t rvWeaponRocketLauncher::State_Fire ( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			nextAttackTime = gameLocal.time + 5000 + (fireRate * owner->PowerUpModifier ( PMOD_FIRERATE ));		
			Attack ( false, 20, spread * 20, 0, 1.0f*10 );
			//bl233[1] - increased damage, spread, and number of attacks
