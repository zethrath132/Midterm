...
stateResult_t rvWeaponHyperblaster::State_Fire ( const stateParms_t& parms ) {
	...
	switch ( parms.stage ) {
		case STAGE_INIT:
			...
			//bl233[13] - increases spread based on the amount of kills the player obtains
			Attack ( false, 1, spread+gameLocal.mpGame.playerState[gameLocal.GetLocalPlayer() -> entityNumber].fragCount, 0, 1.0f );
			if ( ClipSize() ) {
				viewModel->SetShaderParm ( HYPERBLASTER_SPARM_BATTERY, (float)AmmoInClip()/ClipSize() );
			} else {
				viewModel->SetShaderParm ( HYPERBLASTER_SPARM_BATTERY, 1.0f );		
